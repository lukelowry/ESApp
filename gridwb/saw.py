...
class SAW(object):
    ...
    def change_and_confirm_params_multiple_element(
        self, ObjectType: str, command_df: pd.DataFrame
    ) -> None:
        ...
        eq = self._df_equiv_subset_of_other(
            df1=cleaned_df, df2=df, ObjectType=ObjectType
        )
        ...
        if not eq:
            m = (
                "After calling ChangeParametersMultipleElement, not all "
    ...
    def _clean_df(self, ObjectType, fields, obj, df_flag):
        ...
        obj[nn_cols] = (
            obj[nn_cols].apply(lambda x: x.str.strip())
            if df_flag
            else obj[nn_cols].str.strip()
        )
    ...
    def get_key_fields_for_object_type(self, ObjectType: str) -> pd.DataFrame:
        ...
        key_field_df["key_field"] = key_field_df["key_field"].str.replace(
            r"\*", "", regex=True
        )
        ...
        key_field_df.set_index(
            keys="key_field_index", drop=True, verify_integrity=True, inplace=True
        )
    ...
    def get_power_flow_results(
        ...
        try:
            ...
            raise ValueError(
                f"Unsupported ObjectType for power flow results, {ObjectType}."
            ) from e

        return self.GetParametersMultipleElement(
            ObjectType=object_type, ParamList=field_list
        )
    ...
    def identify_numeric_fields(
        self, ObjectType: str, fields: Union[List, np.ndarray]
    ) -> np.ndarray:
        ...
        try:
            ...
            if set(ifn) != set(fields):
                raise ValueError(
                    "The given object has fields which do not"
    ...
    def set_simauto_property(
        self, property_name: str, property_value: Union[str, bool]
    ):
    ...
    def get_branch_admittance(self):
        ...
        branch = self.GetParametersMultipleElement(
            "branch",
            self.get_key_field_list("branch")
        ...
        Ys = 1 / (
            branch["LineR"].to_numpy() + 1j * branch["LineX"].to_numpy()
        )  # series admittance
    ...
    def get_shunt_admittance(self):
        ...
        """
        base = (
            self.GetParametersMultipleElement("Sim_Solution_Options", ["SBase"])
            .to_numpy(float)
            .ravel()
        )
        key = self.get_key_field_list("bus")
    ...
    def get_jacobian(self, full=False):
        ...
        :param full: Convert the csr_matrix to the numpy array (full matrix).
        """
        jacfile = tempfile.NamedTemporaryFile(mode="w", suffix=".m", delete=False)
        ...
        exp = re.compile(r"(?:Jac\()({ie}),({ie})(?:\)=)({fe})".format(ie=ie, fe=fe))
        row = []
        col = []
        data = []
        # Get the dimension from the first line in lines
    ...
    def to_graph(
        ...
        if node not in ["bus", "substation"]:
            raise ValueError(
                "Currently only support 'bus' or 'substation' as the node " "type."
            )
        ...
        if node.lower() in "bus":
            ...
            node = "substation"
            qf = self.get_key_field_list("branch") + ["SubNum", "SubNum:1", "LineMW"]
            node_from = "SubNum"
            node_to = "SubNum:1"
            nf = ["SubNum"]
            if geographic:
    ...
    def DetermineBranchesThatCreateIslands(
        ...
        file.close()
        statement = (
            f'DetermineBranchesThatCreateIslands({Filter},{StoreBuses},"{filename}",{SetSelectedOnLines},'
            f"CSV);"
        )
        print(statement)
        self.RunScriptCommand(statement)
    ...
    def DetermineShortestPath(
        ...
        df = pd.read_csv(
            ...
            names=["BusNum", BranchDistanceMeasure, "BusName"],
        )
        df["BusNum"] = df["BusNum"].astype(int)
        return df
    ...
    def get_lodf_matrix(
        ...
        branches_data = self.GetParametersMultipleElement(
            ObjectType="Branch", ParamList=params
        )
        count = (
            branches_data[branches_data["Status"] == "Closed"].shape[0]
            if ignore_open_branch
            else branches_data.shape[0]
        )
        ...
        if raw:
            ...
            if count <= 1000:
                ...
                self._extracted_from_get_lodf_matrix_16(
                    array, precision, ignore_open_branch
                )
        self.pw_order = original
    ...
    def _extracted_from_get_lodf_matrix_16(self, array, precision, ignore_open_branch):
        ...
        for batch in partition_all(20, array):
            ...
            isl = (
                np.any(temp >= 10, axis=1)
                if isl is None
                else np.logical_or(isl, np.any(temp >= 10, axis=1))
            )
    ...
    def get_shift_factor_matrix(self, method: str = "DC"):
        ...
        self.RunScriptCommand(
            f"CalculateShiftFactorsMultipleElement(BRANCH,SELECTED,BUYER,"
            f"[SLACK],{method})"
        )
    ...
    def _prepare_sensitivity(self):
        ...
        Bbus.data[
            Bbus.indptr[slack] : Bbus.indptr[slack + 1]
        ] = 0  # change the slack column to 0
    ...
    def change_to_temperature(
        self, T: Union[int, float, np.ndarray], R25=7.283, R75=8.688
    ):
        """
        ...
        if isinstance(T, np.ndarray):
            ...
            branch.loc[branch["BranchDeviceType"] == "Line", "LineR"] *= 1 + (
                R75 / R25 - 1
            ) / 50 * (T - 25)
    ...
    def run_contingency_analysis(self, option: str = "N-1", validate: bool = False):
        ...
        convert_dict = {"MWFrom": float, "LineLimMVA": float}
        ...
        if validate and not secure:
            if option == "N-1":
                ...
                ctg_ele = (
                    pd.concat([ctg_ele0, ctg_ele1]).sort_index().reset_index(drop=True)
                )
    ...
    def run_robustness_analysis(self):
        ...
        kf = self.get_key_field_list("branch") + [
            "LineMW",
            "LineMVA",
            "LineLimMVA",
            "LineMaxPercent",
        ...
        graph = nx.from_pandas_edgelist(
            ...
            edge_key="LineCircuit",
            edge_attr=["LineMW", "LineLimMVA", "LineMaxPercent", "BranchDeviceType"],
        )
    ...
    def run_ecological_analysis(self, target: str = "MW", split_generator: bool = True):
        ...
        gen_keys = self.get_key_field_list("gen") + [
            "GenMW",
            "GenMVR",
            "GenMVA",
            "GenProdCost",
        ...
        self.pw_order = (
            False  # fix the order of generator list for later matrix organization
        )
    ...
    def n2_fast(self, c1_isl, count, lodf, f, lim):
        ...
        changing = 1
        num_isl_ctg = (
            np.sum(c1_isl.ravel()) * count
            - np.sum(c1_isl.ravel())
            + np.sum(c2_isl.ravel()) / 2
        )
        kmax = 10
        storage = {}
    ...
    def n2_bruteforce(self, count, A0, lodf, lim, f):
        ...
        def _compute_violation(
            lodf, f, c2, lim, i, count, A0, brute_cont, idx
        ):  # pragma: no cover
        ...
        for i in trange(count - 1):
            if np.sum(A0[i, :] > 0):
                k = compute_violation(lodf, f, c2, lim, i, count, A0, brute_cont, k)
        print(
    ...
    def ctg_solveall(self):
        ...
        return self.GetParametersMultipleElement(
            "Contingency", ["Name", "Solved", "Violations"]
        )
    ...
    def ChangeParametersSingleElement(
        self, ObjectType: str, ParamList: list, Values: list
    ) -> None:
    ...
    def ChangeParametersMultipleElement(
        self, ObjectType: str, ParamList: list, ValueList: list
    ) -> None:
            ...
            try:
                ...
                if (r1 is None) or (r2 is None):
                    if r3 is None:
                        ...
                        output = pd.DataFrame(
                            result_arr, columns=self.FIELD_LIST_COLUMNS_NEW
                        )
                else:
                    # If we made it here, use the older columns.
                    output = pd.DataFrame(
                        result_arr, columns=self.FIELD_LIST_COLUMNS_OLD
                    )
    ...
    def GetParametersSingleElement(
        self, ObjectType: str, ParamList: list, Values: list
    ) -> pd.Series:
    ...
    def GetSpecificFieldList(
        self, ObjectType: str, FieldList: List[str]
    ) -> pd.DataFrame:
        """
    ...
    def OpenCase(self, FileName: Union[str, None] = None) -> None:
        ...
        if FileName is None:
            if self.pwb_file_path is None:
                raise TypeError(
                    "When OpenCase is called for the first "
    ...
    def OpenCaseType(
        self, FileName: str, FileType: str, Options: Union[list, str, None] = None
    ) -> None:
        """
    ...
    def SaveCase(self, FileName=None, FileType="PWB", Overwrite=True):
        ...
        if FileName is not None:
            ...
            raise TypeError(
                "SaveCase was called without a FileName, but "
    ...
    def OpenOneLine(
        ...
        script = 'OpenOneline("{}", {}, {} {})'.format(
            filename, view, FullScreen, ShowFull, LinkMethod, Left, Top, Width, Height
        )
    ...
    def ProgramInformation(self) -> Union[tuple, bool]:
        ...
        try:
            ...
            result[0][2] = datetime.datetime.fromtimestamp(
                result[0][2].timestamp(), tz=result[0][2].tzinfo
            )
    ...
    def _call_simauto(self, func: str, *args):
        ...
        try:
            ...
            raise AttributeError(
                f"The given function, {func}, is not a valid SimAuto function."
            ) from None
    ...
    def _df_equiv_subset_of_other(
        self, df1: pd.DataFrame, df2: pd.DataFrame, ObjectType: str
    ) -> bool:
...
def df_to_aux(fp, df, object_name: str):
    ...
    container = [container[0]] + [
        "    " + ls for ls in container[1:]
    ]  # add tab to each line

    # write the remaining part
    container.append("{")
    container.extend(
        json.dumps(row, separators=(" ", ": "))[1:-1] for row in df.values.tolist()
    )
    container.append("}\r\n")
    fp.write("\n".join(container))
