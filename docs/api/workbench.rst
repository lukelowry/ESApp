...
The underlying :class:`~esapp.saw.SAW` connection is available as ``pw.saw``
for direct SimAuto calls. ``PowerWorld.open()`` opens the configured case;
``pw.saw`` is ``None`` until a case is opened.
