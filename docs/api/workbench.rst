...
Case Management
    Loading, saving, and modifying PowerWorld cases (.pwb files)
Simulation Control
    Running power flow, contingency analysis, optimization, and other studies
Data Access
    Reading component data (buses, generators, lines, etc.) and writing modifications
Analysis Apps
    Specialized tools for network analysis, GIC calculations, and other advanced features
PowerWorld Interface
    Underlying connection to PowerWorld Simulator via the SAW (SimAuto Wrapper) class
...
``wb.network``
    Network topology and matrix extraction
``wb.gic``
    Geomagnetically induced current analysis
``wb.esa``
    Low-level SAW (SimAuto Wrapper) interface for advanced usage
...
Simulation
    ``pflow()``, ``reset()``, ``mode()``
Modification
    ``set_gen()``, ``set_load()``, ``open_branch()``, ``close_branch()``
Analysis
    ``auto_insert_contingencies()``, ``solve_contingencies()``, ``find_violations()``
Matrices
    ``ybus()``, ``ptdf()``, ``lodf()``
Data Export
    ``save()``, ``load_aux()``, ``load_script()``
