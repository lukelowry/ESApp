...
This section describes the mathematical model built by :meth:`~esapp.apps.gic.GIC.model`,
...
:attr:`~esapp.apps.gic.GIC.G` property.
...
:math:`(P_H + N_t^{-1} P_L)` is accessible via the :attr:`~esapp.apps.gic.GIC.eff` property.
...
This is the **H-matrix**, accessible via the :attr:`~esapp.apps.gic.GIC.H` property.
...
This is the :math:`\zeta` operator accessible via the :attr:`~esapp.apps.gic.GIC.zeta` property.
...
- :class:`~esapp.apps.gic.GIC` -- GIC analysis and model interface
- :meth:`~esapp.apps.gic.GIC.model` -- Build the conductance network model
