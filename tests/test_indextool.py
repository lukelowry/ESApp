"""
Unit tests for the IndexTool class, using pytest for clarity and robustness.
...
def get_all_gobject_subclasses() -> List[Type[components.GObject]]:
    ...
    all_subclasses = []
    q = list(components.GObject.__subclasses__())
    visited = set(q)
    while q:
        cls = q.pop(0)
...
def test_setitem_broadcast(idx_tool: IndexTool, g_object: Type[components.GObject]):
    ...
    if not unique_keys:  # Keyless object
        idx_tool[g_object, field_to_set] = value_to_set
...
def test_getitem_specific_fields(idx_tool: IndexTool, g_object: Type[components.GObject]):
    ...
    # Act
    result_df = idx_tool[g_object, [field_to_request]]
...
def test_setitem_broadcast_multiple_fields(idx_tool: IndexTool, g_object: Type[components.GObject]):
    ...
    # Act
    idx_tool[g_object, fields_to_set] = values_to_set
